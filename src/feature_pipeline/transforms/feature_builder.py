...
class FeatureBuilder:
    ...
    def build(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["ma_deviation_6"] = (
            df["price"] / grouped["price"].transform(lambda x: x.rolling(6).mean()) - 1
        )
        df["ma_deviation_12"] = (
            df["price"] / grouped["price"].transform(lambda x: x.rolling(12).mean()) - 1
        )
        df["ma_deviation_24"] = (
            df["price"] / grouped["price"].transform(lambda x: x.rolling(24).mean()) - 1
        )

        df["volatility_6"] = grouped["return"].transform(lambda x: x.rolling(6).std())
        df["volatility_12"] = grouped["return"].transform(lambda x: x.rolling(12).std())
        df["volatility_24"] = grouped["return"].transform(lambda x: x.rolling(24).std())
        ...
        if self.include_target:
            ...
            future["timestamp"] = future["timestamp"] - pd.Timedelta(
                hours=self.future_horizon
            )
