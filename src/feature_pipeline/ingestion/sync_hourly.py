...
def batch_load_coin_data(n_coins: int, currency: str) -> None:
    ...
    for coin_id in tqdm(
        coin_ids, total=len(coin_ids), desc="Syncing Coin data from the last two days"
    ):
        url = client.build_url(coin_id)

        params = client.build_params(
            vs_currency=currency, starting_date=starting_date, ending_date=ending_date
        )
    ...
    print(
        f"Successfully fetched hourly data of the two days for "
