...
class CoinGeckoClient:
    def __init__(
        self, base_url: str, endpoint: str, timeout: int = 20, n_retries: int = 3
    ) -> None:
    ...
    def build_params(
        self, vs_currency: str, starting_date: datetime, ending_date: datetime
    ) -> dict:
        params = {
    ...
    def build_chunks(
        self, starting_date: datetime, ending_date: datetime
    ...
    def fetch_coin_data(self, url: str, params: dict, headers: dict) -> dict:
        ...
        for attempt in range(self.n_retries):
            try:
                ...
                wait_time = attempt**2
                print(
                    f"\n ReadTimeout occured. Retry {attempt + 1}/{self.n_retries}"
                    f" after {wait_time}s..."
                )
                ...
                wait_time = attempt**2
                print(
                    f"\n Request failed: {e}. Retry {attempt + 1}/{self.n_retries} "
                    f"after {wait_time}s..."
                )
                time.sleep(wait_time)
